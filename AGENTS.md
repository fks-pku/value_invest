...
- BOM metric-history lock: `bom-step-metrics` / `bom-step-logic` must not render metric names, concrete historical values, `metric-data-table`, `metric-trend-gap`, history/current/future data rows, or visual chart elements. Metric selection and concrete values begin only inside each `bom-stage-integrated-card` under the `Metric 历史与现状` subcard. A same-metric historical data table should collect at least five cutoff-visible, same-definition data points whenever possible. If the same metric spans multiple companies, render comparable company series as tables, not SVG lines. If fewer than five same-metric historical data points are available, show a visible `metric-trend-gap` with the point count and any available data rows rather than drawing a misleading line.
- Metric-choice display lock: inside each `bom-stage-integrated-card` / `Metric 历史与现状`, the `metric-choice-table` under `本环节应看哪些 Metric` must fit within the current card width and wrap text fully. Do not rely on horizontal scrolling for this table; reserve `table-scroll` horizontal overflow for dense data tables, source indexes, target tables, and multi-entity expectation tables.
