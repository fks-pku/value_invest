...
- Current effective priority is the S-curve simplification lock. Spend roughly 80% of research effort on finding the S curve itself: new-technology prospect, feasibility, future industry space, adoption inflection, and whether the trend is becoming irreversible. `行业概况` now requires three public modules by default: `S曲线与产业空间`, `技术链与BOM呈现`, and `关键变量与待验证数据`. `技术链与BOM呈现` is presentation-only at this stage: explain upstream/midstream/downstream, who is on the chain, what each company/node receives, what it produces, and whom it supplies. `竞争格局与利润池` and `瓶颈点` are no longer mandatory public modules; render or research them only when the user explicitly asks for the next deepening round or when a domain playbook marks them as required for a specific decision. Older references below to five mandatory industry modules are superseded by this S-curve simplification lock.
...
- Effective default report content is simplified around S-curve discovery. In `行业概况`, render `S曲线与产业空间` before or alongside `技术链与BOM呈现`, then `关键变量与待验证数据`. `竞争格局与利润池` and `瓶颈点` are optional follow-up deepening modules, not required for the first S-curve report. Do not force every BOM node through competition/share/profit-pool/chokepoint cards in the first pass.
