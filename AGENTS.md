...
- BOM child-question search-first execution lock: every `BOM node × seven child question` is its own minimum research unit. Before writing or strengthening the current question verdict, GPT must run a fresh question-level search/source collection pass, record `source_universe_plan` plus direct/Exa-style `exa_search_plan`, parse selected materials against that question's dimensions, and mark missing evidence as an explicit gap. Old local artifacts, broad node-level source pools, and model priors are caches or hypotheses only; they cannot replace the active search pass. Public HTML must keep raw search plans internal but must start each `details.bom-question-card` with a collapsed `bom-question-research-status` card that states evidence status and gap discipline before `bom-question-verdict`.
- BOM seven-question evidence sequence lock: inside every `details.bom-question-card`, public HTML must render `bom-question-research-status` first, then `bom-question-verdict` / `本问结论` only after search/parse/gap status has been established, then organize the answer into four fixed public sections using the shared class contract. `bom-step-metrics` uses title `研究逻辑链条与应看 Metric`: first give substantive research prose for the causal/judgment chain, then render a `bom-logic-chain-table` whose `bom-logic-chain-row` rows map each logic link to the metric(s) to watch, why that metric matters, and the data requirement. `bom-step-history` uses title `逻辑环节逐卡：Metric 历史数据`: render a single-column `bom-logic-stage-stack` with one `bom-logic-stage-card` per logic-chain row, including `bom-stage-history`, `bom-stage-current`, source chips, and either `metric-data-table` or explicit `metric-trend-gap`. `bom-step-future` uses title `市场对这一子部分的未来预期`. `bom-step-mechanism` uses title `第一性原理评估未来趋势` and must separately discuss sustainable and unsustainable/refuting mechanisms.
- BOM metric-history lock: `bom-step-metrics` must not render concrete historical values, `metric-data-table`, `metric-trend-gap`, history/current/future data rows, or visual chart elements. Concrete values begin only in `bom-step-history`. A same-metric historical data table should collect at least five cutoff-visible, same-definition data points whenever possible. If the same metric spans multiple companies, render comparable company series as data tables, not SVG lines. If fewer than five same-metric historical data points are available, show a visible `metric-trend-gap` with the point count and any available data rows rather than drawing a misleading line.
- BOM actual-history lock: `bom-step-history` is for realized historical observations only. If the same company discloses actual revenue/backlog/RPO and also management guidance, consensus forecast, TAM, or target values for the same metric family, do not mix those forward-looking values into the historical curve to reach five points. Put guidance and forecasts in `bom-step-future` or the current/future prose, and keep the history chart actual-only. If actual-only history has fewer than five same-definition points, render a trend gap.
- BOM primary-metric definition lock: every logic link in `bom-logic-chain-table` should choose exactly one primary metric unless the research explicitly needs a comparable multi-company same-definition series. The metric name must be concrete enough to collect: subject/company or source owner + exact disclosed field + unit, such as "Company X commercial RPO ($B)" rather than "hyperscaler budget/RPO". The data requirement must specify subject, field, unit, frequency or period, preferred source, and whether five or more same-definition historical points exist; if not, mark a trend gap. Do not use basket labels such as "usage/workload anchors", "AI budget and committed demand", "order funnel", or "revenue checkpoints" as primary metric names. Supporting indicators may appear in prose as cross-checks, but they must not dilute the primary metric definition.
...
- Nested-structure interaction lock: any nested public report structure must be collapsible by default. Whenever content is nested inside a question, module, method, chain, grid, card stack, table panel, future/expectation panel, mechanism panel, rollup, or repeated same-level card group, render it as a clickable `details` node with a direct `summary`, visible `chevron`, and compact title so users can scan before opening. Long repeated card stacks default collapsed unless a more specific top-level contract explicitly says `open`. In the BOM seven-question framework, every `bom-question-research-status`, `bom-step-card`, `bom-logic-chain-panel`, `bom-logic-stage-card`, `bom-future-card`, `bom-mechanism-card`, and `bom-s-curve-stage-card` must be a collapsed `details` element by default.
- Future-expectation layout lock: inside `bom-step-future` / `市场对这一子部分的未来预期`, repeated `bom-future-card` / `bom-expectation-card` items must render as a single-column full-width stack: one card per row on desktop, tablet, and mobile. Do not place expectation cards in a horizontal row or multi-column grid.
- Future-expectation content lock: each `bom-future-card` / `bom-expectation-card` must first answer `市场现在预期什么？`. When the expectation involves one or more companies, customers, suppliers, or third-party institutions, render a comparison table as the main body with columns `公司 / 机构`, `现状期间`, `现状口径 / 数值`, `指引期间`, `预期 / 指引口径 / 数值`, and `口径说明 / 投资含义`; do not use scattered field cards as the primary presentation. The current-state column should use the closest same-metric historical baseline for the guidance metric whenever available. If only a proxy is available, explicitly mark the口径 mismatch in `口径说明 / 投资含义`. Current usage, realized revenue, backlog, RPO, product roadmaps, or monitoring metrics must be labeled as baseline, validation, roadmap, or gap; do not present them as market expectation itself unless the source gives explicit guidance, forecast, consensus, TAM, or target values.
- BOM S-curve stage rollup lock: a BOM's S-curve stage may be evaluated only after all seven child questions for that same BOM have completed their search/parse/gap pass and question verdicts. Public HTML must render one collapsed `details.bom-s-curve-stage-card` after the seven `bom-question-card` nodes, not inside an individual child question. The rollup must synthesize the seven searched answers and include `bom-stage-source-discipline`, `bom-stage-current`, `bom-stage-evidence-grid`, `bom-stage-next-signal`, and `bom-stage-downgrade-signal`. If any child question lacks fresh search evidence, the stage confidence must be downgraded or marked pending rather than asserted from local cached material.
