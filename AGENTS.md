...
- BOM six-question evidence sequence lock: inside every `details.bom-question-card`, public HTML must render `bom-question-research-status` first, then `bom-question-verdict` / `本问结论` only after search/parse/gap status has been established, then organize the answer as a stage-flow using the shared class contract. `bom-step-metrics` / `bom-step-logic` uses title `具体逻辑链条`: generate the concrete causal/judgment chain only, without listing metric names, values, tables, or charts. After `01`, render one collapsed `details.bom-stage-integrated-card` per logic-chain row, numbered `02`, `03`, etc. Each integrated stage card must combine three nested subcards: `Metric 历史与现状`, `市场的未来预期`, and `第一性原理评估`. The stage card reuses its exact `01` logic-chain row, independently decides which metric(s) answer that link, shows historical/current data with `metric-data-table` or explicit `metric-trend-gap`, then shows the aligned market expectation/guidance/forecast/TAM/target table, then gives the first-principles support/refutation for that same link. After all stage cards, render one collapsed `details.bom-step-final-trend` titled `整体的未来趋势评估` that synthesizes the per-stage evidence into the whole-question future trend judgment.
