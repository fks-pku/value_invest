...
- In `行业概况`, render `01 技术链与BOM呈现` first, then from `02` onward render one large module per BOM node. Every BOM module must embed six collapsible child question cards: `当前 BOM 的需求是否会被 S 曲线放大拉动？`, `供给能否跟上？`, `谁控制供给？`, `是否已经财务兑现？`, `市场是否已定价？`, and `反证是什么？`. Each BOM module is an independent research unit: keep the analysis focused on the current BOM's own demand pull-through and unit/system elasticity, supply, controllers, financial realization, pricing, and refutation. Other BOM nodes may appear only as upstream/downstream validation evidence, not as current-node investment conclusions; their opportunity analysis belongs in their own BOM module. Do not render public `S曲线与产业空间`, `关键变量与待验证数据`, or `下钻 QA` unless the user explicitly asks for those modules.
- BOM child-question search-first execution lock: every `BOM node × six child question` is its own minimum research unit. Before writing or strengthening the current question verdict, GPT must run a fresh question-level search/source collection pass, record `source_universe_plan` plus direct/Exa-style `exa_search_plan`, parse selected materials against that question's dimensions, and mark missing evidence as an explicit gap. Old local artifacts, broad node-level source pools, and model priors are caches or hypotheses only; they cannot replace the active search pass. Public HTML must keep raw search plans internal but must start each `details.bom-question-card` with a collapsed `bom-question-research-status` card that states evidence status and gap discipline before `bom-step-model`.
- BOM per-metric active-research lock: inside every `BOM node × six child question`, GPT must first split the question into concrete logic stages, then design a heterogeneous `metric_candidate_plan` for each logic stage before searching. Candidate metrics should cover different evidence types such as direct demand, usage intensity, task/workflow complexity, enterprise adoption, customer budget/order, BOM pull-through, financial realization, pricing, and refutation as appropriate. Each selected metric gets its own search/source-collection and parsing pass; do not run one broad query and then use whichever metric happens to appear. Public HTML must not expose raw search queries, but the `Metric 历史与现状` subcard must show the chosen metric groups, available historical/current data, source links, and explicit trend gaps.
- BOM six-question evidence sequence lock: inside every `details.bom-question-card`, public HTML must render `bom-question-research-status` first, then organize the answer as a stage-flow using the shared class contract. `details.bom-step-model` / `判断模型` is step `01`: choose the question-specific professional model before logic or verdict. `details.bom-step-logic` / `具体逻辑链条` is step `02`: generate the concrete causal/judgment chain only, without listing metric names, values, tables, or charts. After `02`, render one collapsed `details.bom-stage-integrated-card` per logic-chain row, numbered `03`, `04`, etc. Each integrated stage card must combine three nested subcards: `Metric 历史与现状`, `市场的未来预期`, and `第一性原理评估`. The stage card reuses its exact `02` logic-chain row, independently decides which metric(s) answer that link, shows historical/current data with `metric-data-table` or explicit `metric-trend-gap`, then shows the aligned market expectation/guidance/forecast/TAM/target table, then gives the first-principles support/refutation for that same link. After all stage cards, render collapsed `details.bom-step-question-conclusion` / `本问结论` and then collapsed `details.bom-step-target-impact` / `对标的推荐的影响`. Do not render `bom-question-verdict` before evidence; the conclusion belongs after stage evidence.
- BOM metric-history lock: `bom-step-metrics` / `bom-step-logic` must not render metric names, concrete historical values, `metric-data-table`, `metric-trend-gap`, history/current/future data rows, or visual chart elements. Metric selection and concrete values begin only inside each `bom-stage-integrated-card` under the `Metric 历史与现状` subcard. That subcard should directly render one `metric-history-group` per selected metric, titled `Metric 1`, `Metric 2`, etc., followed by a real `metric-history-table`; a compact metric-candidate/coverage strip is allowed when one logic stage uses multiple heterogeneous metrics. Do not insert generic instruction cards such as `本环节应看哪些 Metric` or prose-first current-reading cards above the data. Each table must put the concrete metric name once in `metric-history-caption` as a blue `metric-history-name` source hyperlink, and keep the metric definition / data requirement once in `metric-history-definition`; do not repeat `Metric` or `口径 / 数据要求` as identical columns on every historical data row. The data rows should only carry changing observations such as subject when row-specific, period/cutoff, actual calendar time, value, relative change, and notes. The table must keep the original reported fiscal/calendar label in `期间 / 截点` and add `实际时间` to map labels such as `Q2 FY2026` into natural time windows whenever the company fiscal calendar is known; fiscal FY/Q labels must not appear alone without the actual-time mapping. A same-metric historical data table should collect at least five cutoff-visible, same-definition data points whenever possible. If the same metric spans multiple companies, render comparable company series as tables, not SVG lines. If fewer than five same-metric historical data points are available, show a visible `metric-trend-gap` with the point count and any available data rows rather than drawing a misleading line.
- BOM actual-history lock: `Metric 历史与现状` is for realized historical observations only. If the same company discloses actual revenue/backlog/RPO and also management guidance, consensus forecast, TAM, or target values for the same metric family, do not mix those forward-looking values into the historical curve to reach five points. Put guidance and forecasts in the same stage card's `市场的未来预期` subcard, and keep the history chart actual-only. If actual-only history has fewer than five same-definition points, render a trend gap.
- BOM primary-metric definition lock: primary metric selection belongs inside each `bom-stage-integrated-card` in `Metric 历史与现状`, not in the `01` logic-chain table. Each logic-chain card should choose exactly one primary metric unless the research explicitly needs a comparable multi-company same-definition series. The metric name must be concrete enough to collect: subject/company or source owner + exact disclosed field + unit, such as "Company X commercial RPO ($B)" rather than "hyperscaler budget/RPO". The data requirement must specify subject, field, unit, frequency or period, preferred source, and whether five or more same-definition historical points exist; if not, mark a trend gap. Do not use basket labels such as "usage/workload anchors", "AI budget and committed demand", "order funnel", or "revenue checkpoints" as primary metric names. Supporting indicators may appear in prose as cross-checks, but they must not dilute the primary metric definition.
- BOM child-card prose quality lock: the four section titles are presentation scaffolds, but the public body text must be research content, not instructions about the scaffold. Do not write phrases that tell the reader what a section will do, such as “这一段/这张卡先讲什么”, “01 只讲”, “02 再展示”, or “本报告会”. Every logic row and stage card must have a clear decision use: demand/workload, customer budget or orders, BOM-specific pull-through, price/capacity, supply control, financial realization, valuation/pricing, or refutation/quality signal. Historical baseline / acceleration comparison is still required when claiming "more", "rapid growth", or "S-curve"; future runway / remaining-space analysis must combine cutoff-visible public forecasts, company/customer guidance, financial execution evidence, and first-principles reasoning. If no cutoff-visible historical baseline or YoY/倍数 evidence exists, write "current snapshot is strong" rather than "accelerating"; if no future metric anchor exists, mark future runway as a gap. Keep source plans, parser traces, and per-source audit grids internal unless the user explicitly asks for the research workbench.
- Nested-structure interaction lock: any nested public report structure must be collapsible by default. Whenever content is nested inside a question, module, method, chain, grid, card stack, table panel, mechanism panel, rollup, or repeated same-level card group, render it as a clickable `details` node with a direct `summary`, visible `chevron`, and compact title so users can scan before opening. Long repeated card stacks default collapsed unless a more specific top-level contract explicitly says `open`. In the BOM six-question framework, every `bom-question-research-status`, `bom-step-card`, `bom-step-model`, `bom-logic-chain-panel`, `bom-stage-integrated-card`, `bom-stage-subcard`, `bom-mechanism-card`, `bom-step-question-conclusion`, `bom-step-target-impact`, and `bom-s-curve-stage-card` must be a collapsed `details` element by default. The `Metric 历史与现状` and `市场的未来预期` subcard bodies may directly render tables because the subcard itself is already the collapsible layer.
- Future-expectation layout lock: inside each `bom-stage-integrated-card` / `市场的未来预期`, render direct `expectation-table-group` blocks rather than another nested `details.bom-future-card`. Use one full-width `bom-expectation-table` per entity / institution expectation, titled `实体 1 预期`, `实体 2 预期`, etc.
- Future-expectation content lock: each `expectation-table-group` must use a comparison table with columns `公司 / 机构`, `现状期间`, `现状实际时间`, `现状口径 / 数值`, `指引期间`, `指引实际时间`, `预期 / 指引口径 / 数值`, and `口径说明 / 投资含义`; do not use scattered field cards or a separate `市场现在预期什么？` prose card as the primary presentation. `现状期间` and `指引期间` preserve the company-reported fiscal/calendar label, while `现状实际时间` and `指引实际时间` map fiscal labels such as `Q2 FY2026` to calendar time such as `2025-10 至 2025-12` whenever the company fiscal calendar is known. If a row mixes multiple entities with different fiscal calendars, explicitly mark it as a mixed-entity aggregate instead of pretending one fiscal calendar applies to all. The `公司 / 机构` column must contain the entity name as a blue source hyperlink. The current-state column should use the closest same-metric historical baseline for the guidance metric whenever available. If only a proxy is available, explicitly mark the口径 mismatch in `口径说明 / 投资含义`. Current usage, realized revenue, backlog, RPO, product roadmaps, or monitoring metrics must be labeled as baseline, validation, roadmap, or gap; do not present them as market expectation itself unless the source gives explicit guidance, forecast, consensus, TAM, or target values. Third-party forecasts may only appear under the closest matching `01` logic-chain row with a口径 caveat; they must not create a separate public expectation stage unless that stage also exists in `01`.
- BOM S-curve stage rollup lock: a BOM's S-curve stage may be evaluated only after all six child questions for that same BOM have completed their search/parse/gap pass and question verdicts. Public HTML must render one collapsed `details.bom-s-curve-stage-card` after the six `bom-question-card` nodes, not inside an individual child question. The rollup must synthesize the six searched answers and include `bom-stage-source-discipline`, `bom-stage-current`, `bom-stage-evidence-grid`, `bom-stage-next-signal`, and `bom-stage-downgrade-signal`. If any child question lacks fresh search evidence, the stage confidence must be downgraded or marked pending rather than asserted from local cached material.
- Top-level order is exactly `当前研究的问题` -> `行业概况` -> `标的推荐` -> `来源索引`.
- Current effective priority is the BOM-first six-question lock. In public HTML, `行业概况` starts with `01 技术链与BOM呈现`, then each BOM node becomes a standalone `details.industry-module.bom-research-module` numbered from `02`. The six child cards inside each BOM node are the default public research scaffold for finding S-curve waves: demand pull-through and unit/system elasticity, supply constraint, supply controller, financial realization, market pricing, and refutation. `S曲线与产业空间`, `关键变量与待验证数据`, `竞争格局与利润池`, and `瓶颈点` are optional follow-up modules; render or research them only when the user explicitly asks for the next deepening round or when a domain playbook marks them as required for a specific decision. Older references below to mandatory industry modules are superseded by this simplification lock.
...
- Frontend table-scroll lock: wide or dense tables inside nested cards must render a visible local horizontal scrollbar through `table-scroll`. Nested card/table parents must allow shrinkage (`min-width:0`) so the scroll area stays inside the opened card and metric values are not clipped by outer `overflow:hidden` containers.
...
3. Specialty parsing
   - Route concrete materials through the relevant leaf skill:
     - `s-curve-investment-research` for S 曲线机会、早期指数增长、BOM 需求传导、供需缺口、稀缺节点和标的观察框架。
