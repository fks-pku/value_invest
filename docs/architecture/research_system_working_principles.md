...
`l3_research_plans/<l3_node_id>/` 拥有独立计划。初始版本只含 L3，L3 本身
就是可执行步骤。先搜集和分析；只有回答门槛失败并留下具体证据缺口时，
才把该缺口变成 L4。L4 仍无法回答时才生成 L5，每次只新增一层：
...
当前状态由事件账本投影出来。材料搜集必须由当前最深未回答问题发起，并携带
L3 计划、问题 ID、问题层级、步骤和 `search_run_id`。IMA 日归档或宽泛材料池只能形成
候选，不能先堆材料再批量映射成多个问题的证据。一个来源若服务多个问题，
必须分别创建问题附件、提取和 GPT 复核。缺数据时必须记录 `step_blocked`、
