...
The BOM question sections are not allowed to describe the template itself. Public body copy must be substantive research prose. `01 判断模型` states the professional model, formula, key questions, evidence handles, and conclusion rule. `02 具体逻辑链条` states the causal chain without metric values. The following stage cards must break the chain into per-link cards rather than one loose metric list. Same-metric historical data tables should collect at least five cutoff-visible, same-definition data points whenever possible; if the same metric spans multiple companies, show comparable company series as data tables rather than multi-line charts. If fewer than five same-metric points are available, render a visible trend gap with point count and any available data rows instead of drawing a misleading line. Avoid process phrases such as “这一段/这张卡先讲什么”, “01 只讲”, “02 再展示”, or “本报告会”.
...
- BOM-first validation: when a report uses `bom-question-stage-flow`, every `bom-question-card` must include collapsed `bom-question-research-status` before `bom-step-model`, then `bom-step-logic`, stage evidence cards, `bom-step-question-conclusion`, and `bom-step-target-impact`; every BOM module must include one collapsed `bom-s-curve-stage-card` only after its six child questions, with stage source discipline, current stage, six-question evidence rollup, next confirmation signal, and downgrade signal.
