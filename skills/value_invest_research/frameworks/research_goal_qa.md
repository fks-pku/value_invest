...
- `行业空间`: direct BOM-node space reasoning only. Current scale anchors are evidence, not the conclusion. Use `industry-space-summary`, `space-bom-reasoning`, and one collapsed `details.space-node-card` per key BOM/subsystem node. Each node card must use `space-node-reasoning` and four fixed blocks: `space-node-evidence` / 证据, `space-node-space-reasoning` / 空间推理, `space-node-risk` / 风险反证, and `space-node-conclusion` / 结论. The 空间推理 block must include `space-node-sizing`, `space-sizing-grid`, and `space-node-sizing-table` for `轻量数值测算`, including `测算公式`, `当前锚点`, `未来假设`, Bear/Base/Bull range or proxy anchor, sources, and confidence. Do not answer competition, profit-pool ownership, valuation, or target ranking in this module.
