...
- `行业空间`: future-space sizing model only. Current scale anchors are evidence, not the conclusion. Use `industry-space-summary`, `space-summary-grid`, nested `details.space-detail-panel`, `space-boundary-grid`, `space-driver-tree`, `space-scenario-table`, `space-sizing-table`, and `space-validation-table`. The sizing table should be BOM/subsystem level: future space judgment, current evidence anchor, expansion mechanism, upper-bound constraint, confidence, sources, and next data. Do not answer competition, profit-pool ownership, valuation, or target ranking in this module.
