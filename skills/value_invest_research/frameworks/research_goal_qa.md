...
`行业概况` is mandatory and is the fact-map layer before QA. Current first-pass reports use three default public modules: `S曲线与产业空间`, `技术链与BOM呈现`, and `关键变量与待验证数据`. Each module renders as a clickable collapsed `details.industry-module > summary.module-head` card with `module-index`, `chevron`, and `industry-module-body`. `技术链与BOM呈现` must keep a readable upstream/midstream/downstream swimlane view, a value-flow view, and a `component-value-chain` so readers can see who provides what before Q1-Q4. These long chain components must render as nested clickable `details.chain-detail-panel` cards. `S曲线与产业空间` reuses the `industry-space` component family and must focus on new-technology prospect, feasibility, adoption inflection, future space, and whether the trend is becoming irreversible. Public sizing methods should prioritize company guidance, company TAM, customer-side guidance, third-party sizing, and financial execution evidence, with visible source chips or explicit gaps. When a public-method entry exists, keep the same card fields: `公司或机构`, `指引内容`, `BOM 节点`, `时间范围`, `可验证指标`, and `置信度`; the `空间结论` must still judge `短期`, `中期`, and `长期` space and state the overall `结论`. Optional follow-up modules `竞争格局与利润池` and `瓶颈点` may be added later, but they are not mandatory in the first S-curve report.
