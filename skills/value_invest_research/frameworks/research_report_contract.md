...
- The first section is chain-only: `bom-step-metrics` / `bom-step-logic` must not contain metric names, concrete historical values, SVG/chart elements, `metric-data-table`, `metric-trend-gap`, or history/current/future data rows. Metric selection and concrete data begin inside each `bom-stage-integrated-card` under `Metric 历史与现状`. This subcard should directly show `Metric 1`, `Metric 2`, etc. and the corresponding `metric-history-table`; each table must put the concrete metric name once in `metric-history-caption` as a blue `metric-history-name` source hyperlink, and keep the strict metric definition / data requirement once in `metric-history-definition`. Do not repeat identical `Metric` or `口径 / 数据要求` columns on every historical data row; data rows should focus on changing observations such as subject when row-specific, period/cutoff, value, relative change, and notes. A same-metric historical data table should collect at least five cutoff-visible, same-definition data points whenever possible. If the same metric spans multiple companies, comparable company series must render as data tables rather than multi-line charts. If fewer than five same-metric points exist, the card must say so visibly with a `metric-trend-gap`, `metric-point-count`, and any available data rows instead of drawing an invented or under-supported trend. If no future anchor exists, mark future runway as a gap.
