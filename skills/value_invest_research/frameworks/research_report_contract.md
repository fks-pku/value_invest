...
Parents have exactly two numbered modules, in order:
...
2. `分析与结论`: answer the parent directly, synthesize the causal connections,
   constraints, contradictions and remaining uncertainty, then show linked child
   findings and their sufficiency/boundaries. Aggregate level by level, with evidence
   traceable to leaves; do not concatenate or flatten all descendant articles.

Leaves have exactly one module, `分析与结论`, with no empty child-question section.
It must read as an independent professional research chapter: core judgment,
decisive data and calculations, substantial causal reasoning, alternative
explanations/refutation, then a bounded answer and contribution to the parent.
...
Both kinds retain a compact article-end sufficiency note with reasons, material
gaps and next action. Do not duplicate the same conclusion in a separate abstract
...
- Authoring follows a readable argument: answer, decisive facts, their implication,
  bounded conclusion. This is not a required heading count or minimum word count.
...
- Optional `analysis[].supplementary: true` marks authored secondary calculations or
  ...
  with its argument. Render child findings after the main synthesis and leaf evidence
  after the article. All supporting content stays pre-rendered, keyboard-accessible,
...
- `validate-report-contract` routes this profile to strict parent-two/leaf-one module, tree-coverage,
