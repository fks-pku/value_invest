...
- In `行业概况`, render `01 技术链与BOM呈现` first, then from `02` onward render one large module per BOM node. Every BOM module must embed seven collapsible child question cards: `需求是否会大幅增长？`, `单位用量是否会提升？`, `供给能否跟上？`, `谁控制供给？`, `是否已经财务兑现？`, `市场是否已定价？`, and `反证是什么？`. Each BOM module is an independent research unit: focus on the current BOM's own demand, unit usage, supply, controllers, financial realization, market pricing, and refutation. Other BOM nodes may appear only as upstream/downstream validation evidence, not as current-node investment conclusions; their opportunity analysis belongs in their own BOM module. Do not render public `S曲线与产业空间`, `关键变量与待验证数据`, or `下钻 QA` unless explicitly requested.
- Every `BOM node × child question` is a minimum research unit. Before writing or strengthening that child-question verdict, GPT must run a fresh question-level search/source collection pass, record `source_universe_plan` plus direct/Exa-style `exa_search_plan`, parse selected materials against the question dimensions, and mark missing evidence as explicit gaps. Old local artifacts, broad node-level source pools, and model priors cannot replace the active search pass. Public HTML keeps raw search plans internal but each `details.bom-question-card` must start with collapsed `bom-question-research-status`, then `bom-question-verdict` / `本问结论`, then render a four-section evidence sequence with fixed public titles and shared classes. `bom-step-metrics` uses `研究逻辑链条与应看 Metric`: first provide substantive causal/judgment prose, then a `bom-logic-chain-table` whose `bom-logic-chain-row` rows map every logic link to the metric(s) to watch, why the metric matters, and the data requirement. `bom-step-history` uses `逻辑环节逐卡：Metric 历史数据`: render a single-column `bom-logic-stage-stack` with one `bom-logic-stage-card` per logic-chain row; each card shows `bom-stage-history`, `bom-stage-current`, source chips, and either `metric-data-table` or explicit `metric-trend-gap`. `bom-step-future` uses `市场对这一子部分的未来预期`. `bom-step-mechanism` uses `第一性原理评估未来趋势` and must separately discuss sustainable and unsustainable/refuting mechanisms. Source chips come after this sequence.
- A BOM's S-curve stage may be evaluated only after all seven child questions for that same BOM have completed search, source parsing, gap marking, and question verdicts. Public HTML must render exactly one collapsed `details.bom-s-curve-stage-card` after the seven `bom-question-card` nodes, not inside an individual child question. The rollup must include `bom-stage-source-discipline`, `bom-stage-current`, `bom-stage-evidence-grid`, `bom-stage-next-signal`, and `bom-stage-downgrade-signal`. If any child question lacks fresh search evidence, stage confidence must be downgraded or marked pending instead of asserted from local cached material.
- The first section is rationale-only: `bom-step-metrics` must not contain concrete historical values, SVG/chart elements, `metric-data-table`, `metric-trend-gap`, or history/current/future data rows. Concrete data begins in `bom-step-history`. A same-metric historical data table should collect at least five cutoff-visible, same-definition data points whenever possible. If the same metric spans multiple companies, comparable company series must render as data tables rather than multi-line charts. If fewer than five same-metric points exist, the card must say so visibly with a `metric-trend-gap`, `metric-point-count`, and any available data rows instead of drawing an invented or under-supported trend. If no future anchor exists, mark future runway as a gap.
- Historical curves must be actual-history only. `bom-step-history` must not mix realized company disclosures with management guidance, consensus forecasts, TAM targets, next-period outlook, or target-model values to satisfy the five-point threshold. Put guidance and forecasts in `bom-step-future` or explanatory prose. If the actual-only history has fewer than five same-definition points, render a visible trend gap.
- Primary metric definition is part of the public contract. Each `bom-logic-chain-row` should name one representative primary metric unless a same-definition multi-company comparison is explicitly intended. The metric label must include the subject/company or source owner, exact disclosed field, and unit; the data requirement must specify subject, field, unit, frequency or period, preferred source, and whether five or more same-definition points can be collected. If the metric cannot meet that standard, render a visible trend gap rather than broadening the definition. Do not use basket labels such as `hyperscaler budget/RPO`, `order funnel`, `usage/workload anchors`, or `revenue checkpoints` as primary metric names; supporting indicators may appear only as explanatory cross-checks.
...
- Current effective priority is BOM-first public presentation with seven investment questions per node. The first-pass public report should keep `01 技术链与BOM呈现` as the map layer, then use numbered per-BOM modules from `02` to test demand growth, unit usage, supply constraint, supply controller, financial realization, market pricing, and refutation. `S曲线与产业空间`, `关键变量与待验证数据`, `竞争格局与利润池`, and `瓶颈点` are optional follow-up modules, not mandatory first-pass public modules.
...
- The canonical report shell is: `hero`, `top-nav`, `goal-card`, `constraint-definition`, `industry-overview-section`, `industry-module`, `module-head`, `module-index`, `industry-module-body`, `supply-chain-section`, `chain-explain`, `chain-research-bridge`, `chain-node-lens`, `chain-plain-summary`, `chain-detail-panel`, `chain-lane-map`, `chain-value-flow`, `chain-simple-flow`, `component-value-chain`, `bom-taxonomy`, `bom-taxonomy-grid`, `bom-taxonomy-card`, `chain-layer-grid`, `chain-layer-card`, `chain-relationship-graph`, `chain-stage-panel`, `chain-company-list`, `chain-company-card`, `chain-chokepoints`, `bom-research-module`, `bom-node-brief`, `bom-question-list`, `bom-question-card`, `bom-question-research-status`, `bom-question-verdict`, `bom-question-four-step`, `bom-step-metrics`, `bom-step-history`, `bom-step-future`, `bom-step-mechanism`, `bom-logic-chain-panel`, `bom-logic-stage-card`, `bom-future-card`, `bom-mechanism-card`, `bom-s-curve-stage-card`, `bom-stage-source-discipline`, `bom-stage-current`, `bom-stage-evidence-grid`, `bom-stage-next-signal`, `bom-stage-downgrade-signal`, `overview-research-unit`, `overview-question-card`, `overview-answer`, `competition-bom-map`, `competition-bom-card`, `competition-question-grid`, `profit-pool-table`, `bottleneck-release-timeline`, `chokepoint-bom-map`, `chokepoint-bom-card`, `chokepoint-question-grid`, `chokepoint-scorecard`, `chain-data-gaps`, `industry-space`, `industry-space-summary`, `space-bom-reasoning`, `space-node-card`, `space-node-reasoning`, `space-node-evidence`, `space-node-space-reasoning`, `space-node-sizing`, `space-method-step`, `space-step-title`, `space-step-index`, `space-public-methods`, `space-method-card-grid`, `space-method-card`, `space-method-card-body`, `space-method-entry`, `space-method-entry-sources`, `space-method-empty`, `space-horizon-conclusion`, `space-horizon-grid`, `space-horizon-card`, `space-node-sizing-table`, `space-step-confidence`, `table-scroll`, `industry-competition`, `industry-chokepoints`, `industry-key-variables`, `qa-card level-1/2/3`, `qa-body`, `qa-block`, `block-title`, `logic-grid`, `logic-card`, `source-chips`, `source-chip`, `more-chip`, `artifact-card`, `target-section`, `target-profit-bridge`, `target-valuation-table`, `target-summary`, `target-odds-model`, `target-odds-table`, `source-collapse`, `source-grid`, and `source-card`.
- BOM question internals must preserve the current four-part component family: collapsed `details.bom-step-card > summary`, collapsed `details.bom-logic-chain-panel > summary`, `bom-logic-chain-table`, `bom-logic-chain-row`, `bom-logic-stage-stack`, collapsed `details.bom-logic-stage-card > summary`, collapsed `details.bom-future-card > summary`, collapsed `details.bom-mechanism-card > summary`, `bom-stage-history`, `bom-stage-current`, `metric-data-table`, `metric-data-rows`, `metric-data-row`, `metric-point-count`, and `metric-trend-gap`. Legacy classes such as `bom-metric-rationale-list` and `bom-history-metric-list` may exist in historical reports, but refreshed reports should use the logic-chain and collapsible stage-card family.
