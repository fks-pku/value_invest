...
5. Industry-overview lock: final reports must include `行业概况` as a public top-level section before `下钻 QA`. This section must use `industry-overview-section` and five clickable `details.industry-module` cards, and must render: `产业链与生态位` with `supply-chain-section`, `chain-explain`, `chain-research-bridge`, `chain-node-lens`, `chain-plain-summary`, nested `details.chain-detail-panel`, `chain-lane-map`, `chain-value-flow`, `chain-simple-flow`, `component-value-chain`, `chain-layer-grid`, `chain-layer-card`, `chain-relationship-graph`, `chain-stage-panel`, `chain-company-list`, and `chain-company-card`; `行业空间` with `industry-space`, `industry-space-summary`, `space-bom-reasoning`, `space-node-card`, `space-node-reasoning`, `space-node-evidence`, `space-node-space-reasoning`, `space-node-sizing`, `space-sizing-grid`, `space-node-sizing-table`, `space-node-risk`, and `space-node-conclusion`; `竞争格局与利润池` with `industry-competition` and `technology-route-matrix`; `瓶颈点` with `industry-chokepoints`, `chain-chokepoints`, and `bottleneck-release-timeline`; and `关键变量与待验证数据` with `industry-key-variables` and `chain-data-gaps`. It is the fact-map input to Q1-Q4, not decorative background. Wide tables and dense card tables must use `table-scroll` horizontal overflow handling.
...
2. `行业空间`: use `industry-space` as a direct BOM-node space reasoning module only. It must include `industry-space-summary` and `space-bom-reasoning`; every key BOM/subsystem node must render as a collapsed `details.space-node-card` with `space-node-reasoning` and four fixed blocks: `space-node-evidence` / 证据, `space-node-space-reasoning` / 空间推理, `space-node-risk` / 风险反证, and `space-node-conclusion` / 结论. The 空间推理 block must include `space-node-sizing`, `space-sizing-grid`, and `space-node-sizing-table` for `轻量数值测算`, showing `测算公式`, `当前锚点`, `未来假设`, Bear/Base/Bull range or proxy anchor, source chips, and confidence. It should show the cutoff-visible evidence, how that evidence implies future node expansion, what would refute the expansion, and the concise node-level conclusion. It must not answer competition, profit-pool ownership, valuation, or target ranking.
