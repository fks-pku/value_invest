...
Every node's title states the question it answers. Its actual children, not its
level number, determine the public modules. This replaces the former four-slot
layout within the shared profile; existing node and section bookmarks remain valid.

Parents have exactly three numbered modules at every level, in order:

1. `研究子问题`: list every direct child once as a linked question, with one sentence
   of authored `why_it_matters` explaining how its angle helps answer the parent.
   Do not nest child articles or grandchildren inside this list.
2. `子问题核心结论`: show each direct child's linked core finding and its
   sufficiency/boundaries, then synthesize their causal connections, constraints
   and contradictions into a bounded parent answer. Do not flatten descendants.
3. `欠缺的方向`: show recorded parent gaps and unresolved direct-child gaps with
   links and next validation actions. Include sufficiency here. If no material gap
   is recorded within scope, say so; never invent gaps or expand the tree for layout.

Leaves have exactly three numbered modules, in order:

1. `核心观点`: directly answer the question with its material boundary.
2. `关键论证`: continuous professional prose connecting decisive data, calculations,
   causal reasoning and refutation to the viewpoint. Keep necessary charts beside
   arguments. Compact sufficiency, gaps and next actions belong at its end.
3. `数据列表`: question-specific observed data/facts, linked sources, dates, original
   locators, effects and limitations. Show a truthful empty state if evidence is absent.
   A required-data/search checklist is not collected data.

No empty child-question module or fourth assessment module is permitted on a leaf.
Use descriptive argument subheadings, not a mechanical sequence of field panels.
Key data and their sources remain visible alongside the prose; a supporting
evidence table retains source/date, original locator, fact, effect and limitations.
Missing evidence is explicit. An evidence table is never a substitute for analysis.
Use comparison, trend or sensitivity charts/tables only when supported and useful;
state units, periods, assumptions and derivations, never fabricate data for layout.

Both kinds retain sufficiency, reasons, material gaps and next actions in their
designated module above. Do not duplicate the same conclusion in a separate abstract
and conclusion panel. Search plans, required-data checklists, acceptance-rule fields,
extraction/review IDs and execution traces stay in structured audit artifacts.
Sources sit beside the supported facts; an overall source index only supplements them.
Unpassed child findings remain visible as tentative findings/gaps, never completed
evidence. Parents may not add a leaf evidence table or pass while a mandatory child
remains unpassed. Analysis and child-purpose wording are authored research/presentation
data, never inferred or generated by the renderer.

Implementation and reuse:

- Authoring follows a readable argument: answer, decisive facts, their implication,
  bounded conclusion. Within the three fixed modules, argument subheading counts
  and word counts are not measures of quality.
  Keep one main comparison; explain changes of task/comparator before using them.
  Explain economic meaning before algebra; distinguish observed facts, derived values
  and hypothetical inputs beside each example. Major caveats and unpassed dependencies
  stay in the main prose, not exclusively in supporting material.
- Optional `analysis[].supplementary: true` marks authored secondary calculations or
  methodology. Default is false; every node must retain substantive main analysis.
  Render main sections in authored order, then their node-level tables and bridge,
  then a compact optional `补充测算与方法说明` disclosure. Only authors choose this
  role; the renderer must not classify, shorten or rewrite research. Keep each table
  with its argument, inside the parent's second or leaf's second module. Render leaf
  evidence only in `数据列表`. All supporting content stays pre-rendered, keyboard-accessible,
  available without JavaScript and expanded in full-reading/print modes.
- The Markdown sidecar preserves both main and supplementary prose and tables in
  the same reading order. Editorial changes must not alter sources, conclusions,
  cutoff, question IDs or gates without an explicit research revision. Human review
  must check whether a reader can restate the facts, inference and unresolved issue;
  structural validation alone cannot certify readability.
...
- `validate-report-contract` routes this profile to strict parent-three/leaf-three module, tree-coverage,
...
preserve the same claims, evidence, conclusions and gaps, and the same three ordered
modules for each node including the root.
