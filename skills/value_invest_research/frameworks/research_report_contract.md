...
- The first section is chain-only: `bom-step-metrics` / `bom-step-logic` must not contain metric names, concrete historical values, SVG/chart elements, `metric-data-table`, `metric-trend-gap`, or history/current/future data rows. Metric selection and concrete data begin inside each `bom-stage-integrated-card` under `Metric 历史与现状`. A same-metric historical data table should collect at least five cutoff-visible, same-definition data points whenever possible. If the same metric spans multiple companies, comparable company series must render as data tables rather than multi-line charts. If fewer than five same-metric points exist, the card must say so visibly with a `metric-trend-gap`, `metric-point-count`, and any available data rows instead of drawing an invented or under-supported trend. If no future anchor exists, mark future runway as a gap.
- Metric-choice table display is locked: the `metric-choice-table` under `本环节应看哪些 Metric` must wrap text and fit inside the current `bom-stage-integrated-card`; it must not depend on horizontal scrolling to reveal hidden columns. Dense realized data tables, expectation comparison tables, target tables, and source indexes may still use horizontal `table-scroll`.
...
- Nested-structure interaction lock: if a public report section renders nested content inside a question, module, method, chain, grid, card stack, table panel, future/expectation panel, mechanism panel, or repeated same-level card group, that nested structure must be a clickable `details` element with a direct `summary`, concise title, and visible `chevron`. Nested structures default collapsed so readers can scan titles before opening details. The only exception is a more specific top-level contract that requires default `open`, such as top-level QA cards. In the BOM stage-flow framework, every `bom-step-card`, `bom-logic-chain-panel`, `bom-stage-integrated-card`, `bom-stage-subcard`, `bom-future-card`, `bom-mechanism-card`, and `bom-step-final-trend` must be collapsed `details` by default.
- Future-expectation presentation lock: each `bom-stage-integrated-card` / `市场的未来预期` subcard must render its `bom-future-card` / `bom-expectation-card` as a single-column full-width card. Future cards must align one-to-one with their `01` logic-chain row; do not introduce a new public expectation grouping that is not present in the logic chain.
- Future-expectation content lock: each `bom-future-card` / `bom-expectation-card` must first show the corresponding `01` logic-chain stage and selected metric, then answer `市场现在预期什么？`. When the expectation involves companies, customers, suppliers, or third-party institutions, render a comparison table as the main body with columns `公司 / 机构`, `现状期间`, `现状口径 / 数值`, `指引期间`, `预期 / 指引口径 / 数值`, and `口径说明 / 投资含义`; do not use scattered field cards as the primary presentation. Use the closest same-metric historical baseline for the guidance metric whenever available; if only a proxy is available, explicitly mark the口径 mismatch. Current usage, realized revenue, backlog, RPO, product roadmaps, or monitoring metrics must be labeled as baseline, validation, roadmap, or gap; they are not market expectations unless the source gives explicit guidance, forecast, consensus, TAM, or target values. Third-party forecasts may only appear under the closest matching `01` logic-chain row with a口径 caveat; they must not create a separate public expectation stage unless that stage also exists in `01`.
