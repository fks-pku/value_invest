...
- In BOM child cards that answer a causal-chain question, especially `需求是否会大幅增长？`, lead with a professional report-style `research-narrative` block instead of public audit/workbench grids. The block must contain a headline conclusion, simple logic flow, then `chain-node-expansion` that expands each causal-chain node one by one. Every chain node must start with `chain-metric-board` / `chain-metric-card` / `metric-trend-chart` / `metric-noncontinuous-chart` / `metric-trend-gap`: define which metrics measure the node, why those metrics matter, available historical/current/future readings, and data-quality caveats. After the metric board, each chain node must show `历史对比`, `当前状态`, `未来推测`, `反证信号`, and `节点结论` with claim-near source links/source chips. Historical baseline / acceleration comparison is required when claiming demand is bigger, faster, or entering an S-curve; future runway / remaining-space analysis must answer which metrics can still grow, by how much, and when they may arrive. Prefer same-metric time series for history; for future, combine cutoff-visible public forecasts, company/customer guidance, financial execution evidence, and first-principles reasoning. If only YoY RPO, orders, backlog, capex, or other proxies are available, label them as proxies. If no cutoff-visible history exists, state that acceleration is not proven rather than imply it; if no future metric anchor exists, mark future runway as a gap. Raw source plans, search queries, parser traces, and per-source audit grids remain internal process artifacts.
