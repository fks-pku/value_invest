...
2. `行业空间`: use `industry-space` as a BOM-level future-space sizing model only. It must include `industry-space-summary`, `space-summary-grid`, and nested clickable `details.space-detail-panel` cards for `space-boundary-grid`, `space-driver-tree`, `space-scenario-table`, `space-sizing-table`, and `space-validation-table`. It should show the future-space question, sizing boundary, demand/BOM driver tree, Bear/Base/Bull scenario path, BOM/subsystem future-space table, cutoff-visible current anchors as evidence, upper-bound constraints, confidence, and validation data. It must not answer competition, profit-pool ownership, valuation, or target ranking.
