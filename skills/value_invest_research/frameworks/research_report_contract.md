...
- Compact public report lock is the highest-priority current default and supersedes older five-section / public-QA references below. Top-level order is exactly `当前研究的问题` -> `行业概况` -> `标的推荐` -> `来源索引`.
- Do not render public `下钻 QA` by default. QA trees, L3-L5 records, source plans, parser outputs, score worksheets, and workbench traces remain internal artifacts unless the user explicitly asks to inspect them.
- In `行业概况`, render `01 技术链与BOM呈现` first, then from `02` onward render one large module per BOM node. Every BOM module must embed six collapsible child question cards: `当前 BOM 的需求是否会被 S 曲线放大拉动？`, `供给能否跟上？`, `谁控制供给？`, `是否已经财务兑现？`, `市场是否已定价？`, and `反证是什么？`. Each BOM module is an independent research unit: focus on the current BOM's own demand pull-through and unit/system elasticity, supply, controllers, financial realization, market pricing, and refutation. Other BOM nodes may appear only as upstream/downstream validation evidence, not as current-node investment conclusions; their opportunity analysis belongs in their own BOM module. Do not render public `S曲线与产业空间`, `关键变量与待验证数据`, or `下钻 QA` unless explicitly requested.
- Every `BOM node × child question` is a minimum research unit. Before writing or strengthening that child-question verdict, GPT must run a fresh question-level search/source collection pass, record `source_universe_plan` plus direct/Exa-style `exa_search_plan`, parse selected materials against the question dimensions, and mark missing evidence as explicit gaps. Old local artifacts, broad node-level source pools, and model priors cannot replace the active search pass. Public HTML keeps raw search plans internal but each `details.bom-question-card` must start with collapsed `bom-question-research-status`, then render a stage-flow evidence sequence with fixed public titles and shared classes. `bom-step-model` / `判断模型` is step `01`: define the question-specific professional model, judging formula, key questions, evidence handles, and conclusion rule before logic or verdict. `bom-step-logic` / `具体逻辑链条` is step `02`: first provide substantive causal/judgment prose, then a `bom-logic-chain-table` whose `bom-logic-chain-row` rows define every logic link without listing metric names or values. After `02`, render one collapsed `details.bom-stage-integrated-card` per logic-chain row, numbered from `03`. Each integrated stage card must include three collapsed subcards: `Metric 历史与现状`, `市场的未来预期`, and `第一性原理评估`. Before evidence collection for that stage, GPT must create a logic-stage-level `metric_candidate_plan` with heterogeneous metrics and then search/parse each selected metric independently; do not run one broad query and use whichever metric appears first. The history subcard directly renders one `metric-history-group` per metric with a `metric-history-table` or explicit `metric-trend-gap`; do not render a generic metric-choice instruction card above the table. The expectation subcard collects market expectations, guidance, forecasts, TAM, consensus, or explicit targets for that same logic link and directly renders one `expectation-table-group` / `bom-expectation-table` per entity. The first-principles subcard separately discusses support and refutation mechanisms for that logic link. After all integrated stage cards, render collapsed `details.bom-step-question-conclusion` / `本问结论`, then collapsed `details.bom-step-target-impact` / `对标的推荐的影响`. Do not render a verdict before evidence.
- A BOM's S-curve stage may be evaluated only after all six child questions for that same BOM have completed search, source parsing, gap marking, and question verdicts. Public HTML must render exactly one collapsed `details.bom-s-curve-stage-card` after the six `bom-question-card` nodes, not inside an individual child question. The rollup must include `bom-stage-source-discipline`, `bom-stage-current`, `bom-stage-evidence-grid`, `bom-stage-next-signal`, and `bom-stage-downgrade-signal`. If any child question lacks fresh search evidence, stage confidence must be downgraded or marked pending instead of asserted from local cached material.
- The first section is chain-only: `bom-step-metrics` / `bom-step-logic` must not contain metric names, concrete historical values, SVG/chart elements, `metric-data-table`, `metric-trend-gap`, or history/current/future data rows. Metric selection and concrete data begin inside each `bom-stage-integrated-card` under `Metric 历史与现状`. This subcard should directly show `Metric 1`, `Metric 2`, etc. and the corresponding `metric-history-table`; a compact metric-candidate/coverage strip may appear when one logic link intentionally uses multiple heterogeneous metrics. Each table must put the concrete metric name once in `metric-history-caption` as a blue `metric-history-name` source hyperlink, and keep the strict metric definition / data requirement once in `metric-history-definition`. Do not repeat identical `Metric` or `口径 / 数据要求` columns on every historical data row; data rows should focus on changing observations such as subject when row-specific, period/cutoff, actual calendar time, value, relative change, and notes. Every `metric-history-table` must keep the original source period in `期间 / 截点` and add `实际时间` to map fiscal labels such as `Q2 FY2026` into calendar windows whenever the fiscal calendar is known; FY/Q labels must not be the only time reference. A same-metric historical data table should collect at least five cutoff-visible, same-definition data points whenever possible. If the same metric spans multiple companies, comparable company series must render as data tables rather than multi-line charts. If fewer than five same-metric points exist, the card must say so visibly with a `metric-trend-gap`, `metric-point-count`, and any available data rows instead of drawing an invented or under-supported trend. If no future anchor exists, mark future runway as a gap.
- Historical curves must be actual-history only. `Metric 历史与现状` must not mix realized company disclosures with management guidance, consensus forecasts, TAM targets, next-period outlook, or target-model values to satisfy the five-point threshold. Put guidance and forecasts in the same stage's `市场的未来预期` subcard. If the actual-only history has fewer than five same-definition points, render a visible trend gap.
- Primary metric definition is part of the public contract but belongs inside each `bom-stage-integrated-card` under `Metric 历史与现状`, not in the `02` logic-chain table. Each logic row should name one representative primary metric unless the research explicitly needs multiple heterogeneous metrics or a same-definition multi-company comparison. The metric label must include the subject/company or source owner, exact disclosed field, and unit; the data requirement must specify subject, field, unit, frequency or period, preferred source, and whether five or more same-definition points can be collected. If the metric cannot meet that standard, render a visible trend gap rather than broadening the definition. Do not use basket labels such as `hyperscaler budget/RPO`, `order funnel`, `usage/workload anchors`, or `revenue checkpoints` as primary metric names; supporting indicators may appear only as explanatory cross-checks.
- The body copy inside the four BOM question sections must be substantive research prose, not template narration. Do not tell readers what the report section is about to do with phrases such as “这一段/这张卡先讲什么”, “01 只讲”, “02 再展示”, or “本报告会”. Every logic row and stage card must have a clear decision use: demand/workload, customer budget or orders, BOM-specific pull-through, supply/price, supply control, financial realization, valuation/pricing, or quality/refutation signal. Historical baseline / acceleration comparison is required when claiming demand is bigger, faster, or entering an S-curve; future runway / remaining-space analysis must answer which metrics can still grow, by how much, and when they may arrive. If only YoY RPO, orders, backlog, capex, or other proxies are available, label them as proxies. If no cutoff-visible history exists, state that acceleration is not proven rather than imply it. Raw source plans, search queries, parser traces, and per-source audit grids remain internal process artifacts.
- Nested-structure interaction lock: if a public report section renders nested content inside a question, module, method, chain, grid, card stack, table panel, mechanism panel, or repeated same-level card group, that nested structure must be a clickable `details` element with a direct `summary`, concise title, and visible `chevron`. Nested structures default collapsed so readers can scan titles before opening details. The only exception is a more specific top-level contract that requires default `open`, such as top-level QA cards. In the BOM stage-flow framework, every `bom-step-card`, `bom-step-model`, `bom-logic-chain-panel`, `bom-stage-integrated-card`, `bom-stage-subcard`, `bom-mechanism-card`, `bom-step-question-conclusion`, and `bom-step-target-impact` must be collapsed `details` by default. `Metric 历史与现状` and `市场的未来预期` subcards may directly render tables because the subcard is already the collapsible layer.
- Future-expectation presentation lock: each `bom-stage-integrated-card` / `市场的未来预期` subcard must directly render `expectation-table-group` blocks rather than another nested `details.bom-future-card`. Use one full-width `bom-expectation-table` per entity / institution expectation, titled `实体 1 预期`, `实体 2 预期`, etc.
- Future-expectation content lock: each `expectation-table-group` must use columns `公司 / 机构`, `现状期间`, `现状实际时间`, `现状口径 / 数值`, `指引期间`, `指引实际时间`, `预期 / 指引口径 / 数值`, and `口径说明 / 投资含义`; do not use scattered field cards or a separate `市场现在预期什么？` prose card as the primary presentation. `现状期间` and `指引期间` keep the fiscal/calendar label from the source, while `现状实际时间` and `指引实际时间` map fiscal labels into calendar windows whenever the company's fiscal calendar is known. If a row mixes companies with different fiscal calendars, it must explicitly say it is a mixed-entity aggregate rather than applying one company's calendar. The `公司 / 机构` cell must contain the entity name as a blue source hyperlink. Use the closest same-metric historical baseline for the guidance metric whenever available; if only a proxy is available, explicitly mark the口径 mismatch. Current usage, realized revenue, backlog, RPO, product roadmaps, or monitoring metrics must be labeled as baseline, validation, roadmap, or gap; they are not market expectations unless the source gives explicit guidance, forecast, consensus, TAM, or target values. Third-party forecasts may only appear under the closest matching `01` logic-chain row with a口径 caveat; they must not create a separate public expectation stage unless that stage also exists in `01`.
...
- Current effective priority is BOM-first public presentation with six investment questions per node. The first-pass public report should keep `01 技术链与BOM呈现` as the map layer, then use numbered per-BOM modules from `02` to test demand pull-through and unit/system elasticity, supply constraint, supply controller, financial realization, market pricing, and refutation. `S曲线与产业空间`, `关键变量与待验证数据`, `竞争格局与利润池`, and `瓶颈点` are optional follow-up modules, not mandatory first-pass public modules.
...
12. Refreshed canonical reports must use the shared component family: `hero`, `top-nav`, `goal-card`, `constraint-definition`, `industry-overview-section`, `industry-module`, `module-head`, `module-index`, `industry-module-body`, `supply-chain-section`, `chain-explain`, `chain-research-bridge`, `chain-node-lens`, `chain-plain-summary`, `chain-detail-panel`, `chain-lane-map`, `chain-value-flow`, `chain-simple-flow`, `component-value-chain`, `bom-taxonomy`, `bom-taxonomy-grid`, `bom-taxonomy-card`, `chain-layer-grid`, `chain-layer-card`, `chain-relationship-graph`, `chain-stage-panel`, `chain-company-list`, `chain-company-card`, `chain-chokepoints`, `overview-research-unit`, `overview-question-card`, `overview-answer`, `overview-answer-prose`, `overview-answer-structured`, `overview-answer-row`, `research-narrative`, `narrative-head`, `logic-flow`, `flow-step`, `historical-comparison`, `history-snapshot-grid`, `history-metric-card`, `history-bar-list`, `history-bar-row`, `history-table`, `future-runway`, `runway-formula`, `runway-formula-card`, `runway-table`, `runway-timeline`, `runway-verdict`, `narrative-prose`, `narrative-data-table`, `investment-takeaway`, `bear-case-box`, `competition-bom-map`, `competition-bom-card`, `competition-question-grid`, `profit-pool-table`, `bottleneck-release-timeline`, `chokepoint-bom-map`, `chokepoint-bom-card`, `chokepoint-question-grid`, `chokepoint-scorecard`, `key-variable-bom-map`, `key-variable-bom-card`, `chain-data-gaps`, `industry-space`, `industry-space-summary`, `space-bom-reasoning`, `space-node-card`, `space-node-reasoning`, `space-node-evidence`, `space-node-space-reasoning`, `space-node-sizing`, `space-method-step`, `space-step-title`, `space-step-index`, `space-public-methods`, `space-method-card-grid`, `space-method-card`, `space-method-card-body`, `space-method-entry`, `space-method-entry-sources`, `space-method-empty`, `space-horizon-conclusion`, `space-horizon-grid`, `space-horizon-card`, `space-node-sizing-table`, `space-step-confidence`, `table-scroll`, `industry-competition`, `industry-chokepoints`, `industry-key-variables`, `qa-card level-1`, `qa-card level-2`, `qa-card level-3`, optional `qa-card level-4`, optional `qa-card level-5`, `qa-body`, `qa-block`, `block-title`, `artifact-card`, `target-section`, `target-profit-bridge`, `target-valuation-table`, `target-odds-model`, `target-odds-table`, `target-table`, and `source-collapse`. Do not introduce alternate public component families unless the user explicitly asks for a frontend redesign.
13. Causal-chain narrative reports must use `chain-node-expansion`, `chain-node-detail`, `chain-node-lens-grid`, and `chain-node-conclusion` inside `research-narrative` when expanding individual chain nodes.
14. Target-table `action_state` cells must render with the canonical status color classes: `state-actionable_long`, `state-watch_only`, or `state-no_action`. Plain uncolored action-state text is contract drift.
15. Historical training/backtest labels are evaluation fields only. They may appear once in the isolated label area of `标的推荐`, never inside QA conclusion prose or Q4 child logic.
16. Every visible L3-L5 research-unit card must include a compact professional-routing strip with selected `Skill`, actual `Execution` status, `Score Component`, and `Decision Use`. `Skill` is the intended specialty lens; `Execution` must come from `skill_output_status` and `fallback_used`, so a selected skill cannot be mistaken for a completed parser run. Full parser traces still belong in internal files unless explicitly requested.
...
- The canonical report shell is: `hero`, `top-nav`, `goal-card`, `constraint-definition`, `industry-overview-section`, `industry-module`, `module-head`, `module-index`, `industry-module-body`, `supply-chain-section`, `chain-explain`, `chain-research-bridge`, `chain-node-lens`, `chain-plain-summary`, `chain-detail-panel`, `chain-lane-map`, `chain-value-flow`, `chain-simple-flow`, `component-value-chain`, `bom-taxonomy`, `bom-taxonomy-grid`, `bom-taxonomy-card`, `chain-layer-grid`, `chain-layer-card`, `chain-relationship-graph`, `chain-stage-panel`, `chain-company-list`, `chain-company-card`, `chain-chokepoints`, `bom-research-module`, `bom-node-brief`, `bom-question-list`, `bom-question-card`, `bom-question-research-status`, `bom-question-stage-flow`, `bom-step-model`, `bom-step-logic`, `bom-logic-chain-panel`, `bom-logic-stage-stack`, `bom-stage-integrated-card`, `bom-stage-subcard`, `bom-stage-history-card`, `bom-stage-future-card`, `bom-stage-mechanism-card`, `bom-step-question-conclusion`, `bom-step-target-impact`, `metric-history-group`, `metric-history-table`, `metric-history-caption`, `metric-history-name`, `metric-history-definition`, `expectation-table-group`, `bom-expectation-table`, `bom-mechanism-card`, `bom-s-curve-stage-card`, `bom-stage-source-discipline`, `bom-stage-current`, `bom-stage-evidence-grid`, `bom-stage-next-signal`, `bom-stage-downgrade-signal`, `overview-research-unit`, `overview-question-card`, `overview-answer`, `competition-bom-map`, `competition-bom-card`, `competition-question-grid`, `profit-pool-table`, `bottleneck-release-timeline`, `chokepoint-bom-map`, `chokepoint-bom-card`, `chokepoint-question-grid`, `chokepoint-scorecard`, `chain-data-gaps`, `industry-space`, `industry-space-summary`, `space-bom-reasoning`, `space-node-card`, `space-node-reasoning`, `space-node-evidence`, `space-node-space-reasoning`, `space-node-sizing`, `space-method-step`, `space-step-title`, `space-step-index`, `space-public-methods`, `space-method-card-grid`, `space-method-card`, `space-method-card-body`, `space-method-entry`, `space-method-entry-sources`, `space-method-empty`, `space-horizon-conclusion`, `space-horizon-grid`, `space-horizon-card`, `space-node-sizing-table`, `space-step-confidence`, `table-scroll`, `industry-competition`, `industry-chokepoints`, `industry-key-variables`, `qa-card level-1/2/3`, `qa-body`, `qa-block`, `logic-grid`, `logic-card`, `source-chips`, `source-chip`, `more-chip`, `artifact-card`, `target-section`, `target-profit-bridge`, `target-valuation-table`, `target-summary`, `target-odds-model`, `target-odds-table`, `source-collapse`, `source-grid`, and `source-card`.
- BOM question internals must preserve the current stage-flow component family: collapsed `details.bom-step-card > summary`, collapsed `details.bom-step-model > summary`, collapsed `details.bom-logic-chain-panel > summary`, `bom-logic-chain-table`, `bom-logic-chain-row`, `bom-logic-stage-stack`, collapsed `details.bom-stage-integrated-card > summary`, collapsed `details.bom-stage-subcard > summary`, collapsed `details.bom-mechanism-card > summary`, collapsed `details.bom-step-question-conclusion > summary`, collapsed `details.bom-step-target-impact > summary`, `metric-history-group`, `metric-history-table`, `metric-history-caption`, `metric-history-name`, `metric-history-definition`, `metric-data-table`, `metric-point-count`, `metric-trend-gap`, `expectation-table-group`, and `bom-expectation-table`. Legacy classes such as `bom-question-verdict`, `bom-step-final-trend`, `bom-question-four-step`, `bom-step-history`, `bom-step-future`, `bom-step-mechanism`, `bom-metric-rationale-list`, `bom-stage-metric-choice`, `metric-choice-table`, `bom-future-card`, `metric-data-rows`, `metric-data-row`, `metric-name-cell`, and `bom-history-metric-list` may exist in historical reports, but refreshed reports should use the direct table stage-flow family without repeated metric-name cells and without pre-evidence verdicts.
...
- Wide tables and dense nested tables must sit inside a visible local `table-scroll` region. The CSS contract must keep `table-scroll` scrollable on the x-axis and set nested card/table parents to shrink within the available width (`min-width:0`) so metric values remain reachable instead of being clipped by outer `overflow:hidden` cards.
